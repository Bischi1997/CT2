...
{
    /// STUDENTS: To be programmed
		uint32_t t_latency = TIM2->CNT;
	
		hal_timer_irq_clear(TIM2, HAL_TIMER_IRQ_UE);
		tim2_interrupt_counter++;

		if (t_latency < min_latency)
		{
			min_latency = t_latency;
		}
		if (t_latency > max_latency)
		{
			max_latency = t_latency;
		}
		sum_latency += t_latency;
...
static uint8_t convert_uint32_t_to_string(char ret_val[], uint32_t value)
{
    return snprintf(ret_val, STRING_LENGTH_FOR_32BIT, "%d", value);
}
